#pragma once
/*
 * blur16.h — RGBA16F two-pass separable Gaussian for filter.blur.lens.
 *
 * A precision-preserving sibling of fx::GaussianBlur (whose scratch is RGBA8,
 * clamping HDR to [0,1]). Exact fixed sigma in px, 3.0-sigma half-width (matches
 * the prototype's _gauss1d), integer per-pixel taps → no shimmer. Type-shared
 * (file-static). Requires blur16.hlsl compiled into lens_shaders.h (BLUR16_SPV).
 * Adapted from line_reconstruct/blur16.h.
 */
...
public:
  ...
  bool init() {
    ...
    state::registerShaderSPV("lens_blur16", BLUR16_SPV, BLUR16_SPV_SIZE,
                             "rgba16float", "write");
    auto cs = gpu::Device::createShaderModuleByName("lens_blur16");
    if (!cs) return false;
