// source.particles.sweep_chamber — motion-vector vertex shader (points).
// Mirrors vs.hlsl's quad construction but carries the particle's per-frame
// screen-space displacement instead of colour: velocity is stored in uv/s,
// so the uv motion this frame is exactly vel·dt (× an artistic scale).
// Respawns re-seed velocity from the field, so no teleport spikes.
...
MotionVsOut main(uint vid : SV_VertexID, uint iid : SV_InstanceID) {
  ...
  o.corner = c;
  o.motion = p.b.xy * dt * scale;   // uv/s → uv/frame
  return o;
}
