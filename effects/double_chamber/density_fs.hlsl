// source.legacy.double_chamber — density splat fragment shader (interactions).
//
...
static const float DC_DENSITY_SIGMA = 0.5;

[shader("pixel")]
float4 main(DcDOut i) : SV_Target0 {
  float r2 = dot(i.corner, i.corner);
  if (r2 > 1.0) discard;                       // round halo, not a square
  float halo = exp(-r2 / (DC_DENSITY_SIGMA * DC_DENSITY_SIGMA));   // 1 at center
