// source.legacy.double_chamber — motion-vector vertex shader for lines
// (tracers + bridgers). Same oriented-quad geometry as line_vs.hlsl, but the
// per-pixel motion vector points ALONG the segment (its own tangent) scaled by
// `line_speed`: a line "moves" lengthwise, so downstream motion blur smears it
// along its body rather than across it. (The endpoints themselves jump frame
// to frame — a raw endpoint delta would be noise; the tangent is the intent.)
